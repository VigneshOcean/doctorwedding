...
<title>Doctor Life Matrimony:UserId Search</title>
...
if($mydob!='') {
    if($mygender=='male'){
    $condition=" and STR_TO_DATE(REPLACE(dob,'-','/'),'%d/%m/%Y')>=STR_TO_DATE('".$mydob."','%d/%m/%Y')";    
    } else {
        $condition=" and STR_TO_DATE(REPLACE(dob,'-','/'),'%d/%m/%Y')<=STR_TO_DATE('".$mydob."','%d/%m/%Y')";
    }
}
$result=mysqli_query($con, "select * from register where  username='$userid1' and gender!='$mygender' and religion='$uu_caste' and id!='$id' and status='1'  ".$condition)or die(mysqli_error($con));
