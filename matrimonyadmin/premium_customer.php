...
<head>
    <meta charset="utf-8" />
    <title>Doctor Life Matrimony: Premium Customer</title>
    <meta name="description" content="Premium registered profiles" />
    ...
    function stateChangedga() {
        if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete") {
            window.location='premium_customer.php';
        }
    }
        ...
        <div class="main-container-inner">
            ...
            <div class="main-content">
                <div class="page-content">
                    <div class="page-header"><h1>Premium Customer</h1></div>
                    <div class="row">
                        <div class="col-xs-12">
<?php
$tableName  = "register";
$targetpage = "premium_customer.php";
$limit = 10;
$page  = isset($_GET['page']) ? max(1, (int)$_GET['page']) : 1;
$start = ($page - 1) * $limit;
$count_res = mysqli_query($con, "SELECT COUNT(*) as num FROM $tableName WHERE premium_customer='1'");
$total_count = 0;
if ($count_res) {
    $count_row = mysqli_fetch_assoc($count_res);
    $total_count = (int)($count_row['num'] ?? 0);
}
$result = mysqli_query($con, "SELECT * FROM $tableName WHERE premium_customer='1' ORDER BY id DESC LIMIT $start, $limit");
...
?>
                            <form enctype="multipart/form-data" method="post" action="goto_search.php" name="frm">
                                <input id="command" type="hidden" name="command" value="premium_customer.php">
...
?>
                                        <tr>
                                            ...
                                            <td><a href="delete_profile.php?id=<?php echo $e_id; ?>&pagename=premium_customer.php" onClick="return confirm('Are you sure you want to delete?')"><button class="btn btn-xs btn-danger"><i class="icon-trash bigger-120"></i></button></a></td>
                                        </tr>
<?php
    }
} else {
