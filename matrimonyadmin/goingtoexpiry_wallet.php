...
<head>
    ...
    function stateChangedga() {
        if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete") {
            window.location='goingtoexpiry_wallet.php';
        }
    }
        ...
        <div class="main-container-inner">
            ...
            <div class="main-content">
                <div class="page-content">
                    <div class="page-header"><h1>Expiring Wallets</h1></div>
                    <div class="row">
                        <div class="col-xs-12">
<?php
...
$tableName  = "register";
$targetpage = "goingtoexpiry_wallet.php";
$limit = 10;
$page  = isset($_GET['page']) ? max(1, (int)$_GET['page']) : 1;
$start = ($page - 1) * $limit;
$query_count = "SELECT COUNT(*) as num FROM $tableName WHERE wallet_validity_end_string!='' AND (wallet_validity_end_string>=$current_date_string AND wallet_validity_end_string<=$nex_week_string)";
$count_res   = mysqli_query($con, $query_count);
$total_count = 0;
if ($count_res) {
    $count_row   = mysqli_fetch_assoc($count_res);
    $total_count = (int)($count_row['num'] ?? 0);
}
$query_data = "SELECT * FROM $tableName WHERE wallet_validity_end_string!='' AND (wallet_validity_end_string>=$current_date_string AND wallet_validity_end_string<=$nex_week_string) ORDER BY wallet_validity_end_string ASC LIMIT $start, $limit";
...
?>
                            <form enctype="multipart/form-data" method="post" action="goto_search.php" name="frm">
                                <input id="command" type="hidden" name="command" value="goingtoexpiry_wallet.php">
                            ...
                            <span style="font-weight:bold; color:#006600; margin-left:10%; font-size:18px;">Total number of records : <?php echo $total_count; ?> Wallets </span>
                            <div class="row"><div class="col-xs-12"><div class="table-responsive">
                                <table id="sample-table-2" class="table table-striped table-bordered table-hover">
                                    <thead>
                                        <tr>
                                            <th>User Name</th><th>User ID</th><th>Mobile Number</th><th>Wallet Amount</th><th>Wallet Balance</th><th>From Date</th><th>Expiry date</th><th>Action</th>
                                        </tr>
                                    </thead><tbody>
<?php
if ($result && mysqli_num_rows($result) > 0) {
    while ($row_e = mysqli_fetch_assoc($result)) {
        $e_id    = $row_e['id'];
        $w_query = "SELECT * FROM wallet_history WHERE user_id='$e_id' ORDER BY id DESC LIMIT 1";
        $w_res   = mysqli_query($con, $w_query);
        $w_row   = ($w_res && mysqli_num_rows($w_res) > 0) ? mysqli_fetch_assoc($w_res) : [];
?>
                                        <tr>
                                            <td><?php echo ucwords((string)($row_e['name'] ?? '')); ?></td>
                                            <td><?php echo $row_e['username'] ?? ''; ?></td>
                                            <td><?php echo $row_e['mobile'] ?? ''; ?></td>
                                            <td><?php echo $w_row['amount'] ?? ''; ?></td>
