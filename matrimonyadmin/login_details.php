...
<head>
    <meta charset="utf-8" />
    <title>Doctor Life Matrimony: Login Details</title>
        ...
        <div class="main-container-inner">
            ...
            <div class="main-content">
                <div class="page-content">
                    <div class="page-header"><h1>Login Details</h1></div>
                    <div class="row">
                        <div class="col-xs-12">
<?php
$tableName  = "register";
...
$start = ($page - 1) * $limit;
$count_res = mysqli_query($con, "SELECT COUNT(*) as num FROM $tableName WHERE last_login IS NOT NULL");
$total_count = 0;
if ($count_res) {
    $count_row = mysqli_fetch_assoc($count_res);
    $total_count = (int)($count_row['num'] ?? 0);
}
$result = mysqli_query($con, "SELECT * FROM $tableName WHERE last_login IS NOT NULL ORDER BY last_login DESC LIMIT $start, $limit");
...
?>
                            <form enctype="multipart/form-data" method="post" action="goto_search.php" name="frm">
                                <input id="command" type="hidden" name="command" value="login_details.php">
                            ...
                            <div class="row"><div class="col-xs-12"><div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover">
                                    <thead>
                                        <tr>
                                            <th>Name</th><th>Email</th><th>Mobile Number</th><th>Username</th><th>Print Count</th><th>Customer Type</th><th>Last Login</th><th>Last Logout</th><th>Login Status</th>
                                        </tr>
                                    </thead><tbody>
<?php
...
?>
                                        <tr>
                                            ...
                                            <td><?php echo (!empty($row_e['valid_for'])) ? "Paid Customer" : "Non Paid Customer"; ?></td>
                                            <td><?php echo $row_e['last_login'] ?? ''; ?></td>
                                            <td><?php echo $row_e['last_logout'] ?? ''; ?></td>
                                            <td><?php echo (($row_e['login_status'] ?? 0) == 1) ? 'Online' : 'Offline'; ?></td>
                                        </tr>
<?php
    }
} else {
