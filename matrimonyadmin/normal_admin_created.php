...
<head>
    <meta charset="utf-8" />
    <title>Doctor Life Matrimony: Admin Created Profile</title>
    <meta name="description" content="Profiles created by admin" />
    ...
    function stateChangedga() {
        if (xmlHttp.readyState==4 || xmlHttp.readyState=="complete") {
            window.location='normal_admin_created.php';
        }
    }
        ...
        <div class="main-container-inner">
            ...
            <div class="main-content">
                <div class="page-content">
                    <div class="page-header"><h1>Admin Created Profiles</h1></div>
                    <div class="row">
                        <div class="col-xs-12">
<?php
$tableName  = "register";
$targetpage = "normal_admin_created.php";
$limit = 10;
$page  = isset($_GET['page']) ? max(1, (int)$_GET['page']) : 1;
$start = ($page - 1) * $limit;
$count_res = mysqli_query($con, "SELECT COUNT(*) as num FROM $tableName WHERE profile='admin'");
$total_count = 0;
if ($count_res) {
    $count_row = mysqli_fetch_assoc($count_res);
    $total_count = (int)($count_row['num'] ?? 0);
}
$result = mysqli_query($con, "SELECT * FROM $tableName WHERE profile='admin' ORDER BY id DESC LIMIT $start, $limit");
...
?>
                            <form enctype="multipart/form-data" method="post" action="goto_search.php" name="frm">
                                <input id="command" type="hidden" name="command" value="normal_admin_created.php">
                            ...
                            <div class="row"><div class="col-xs-12"><div class="table-responsive">
                                <table id="sample-table-2" class="table table-striped table-bordered table-hover">
                                    <thead>
                                        <tr>
                                            <th>Name</th><th>Email</th><th>Mobile Number</th><th>Dob/Age</th><th>Caste/SubCaste</th><th>Action</th><th>Delete</th>
                                        </tr>
                                    </thead><tbody>
<?php
...
?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php if(($row_e['status'] ?? 0) == '1'){ ?>
                                                <a href="#" onClick="getssts(<?php echo $e_id; ?>)" title="Click to deactivate"><button class="btn btn-xs btn-success"><i class="icon-unlock  bigger-120"></i></button></a>
                                                <?php } else { ?>
                                                <a href="#" onClick="getssts(<?php echo $e_id; ?>)" title="Click to Activate"><button class="btn btn-xs btn-success"><i class="icon-lock  bigger-120"></i></button></a>
                                                <?php } ?>
                                                <a href="edit.php?userid=<?php echo $e_id; ?>&pagename=normal_admin_created.php" target="_blank" title="View/Edit Profile"><button class="btn btn-xs btn-info"><i class="icon-edit bigger-120"></i></button></a>
                                            ...
                                            <td><a href="delete_profile.php?id=<?php echo $e_id; ?>&pagename=normal_admin_created.php" onClick="return confirm('Are you sure you want to delete?')"><button class="btn btn-xs btn-danger"><i class="icon-trash bigger-120"></i></button></a></td>
                                        </tr>
<?php
    }
} else {
